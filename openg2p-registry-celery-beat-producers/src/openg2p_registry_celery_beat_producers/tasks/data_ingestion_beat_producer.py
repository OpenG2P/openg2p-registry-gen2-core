...
@celery_app.task(name="data_ingestion_beat_producer")
def data_ingestion_beat_producer():
    ...
    with session_maker() as session:
        ...
        for incoming_classified_datum in incoming_classified_data:
            ...
            _logger.info(
                f"Updating status for {Workers.DATA_INGESTION_WORKER} to processing for incoming_classified_data with ingest_id: {incoming_classified_datum.ingest_id}"
            )

            # Send task to appropriate celery worker
            celery_app.send_task(
                Workers.DATA_INGESTION_WORKER,
                args=(incoming_classified_datum.ingest_id,),
                queue=_config.worker_queue,
            )
            _logger.info(
                f"Sent task to {Workers.DATA_INGESTION_WORKER} for incoming_classified_data with ingest_id: {incoming_classified_datum.ingest_id}"
            )
        session.commit()
